...
solisteDynamics = {
  ...
  \markCustom "C"
  \partial 4 s4 s1*5 s2. s4\mf s1 s4-\markup{\italic rall.}\> s2. s2\!\p s2 s4 s4\sf s2 s1*2 \bar "||" \break % La mort et la vie…
  \markCustom "D"
  \partial 4 s4 s1*5 s2. s4\mf s1*3 s2. s4\f s1*4 \bar "||" \break % Dis-nous Marie Madeleine…
  \markCustom "E"
